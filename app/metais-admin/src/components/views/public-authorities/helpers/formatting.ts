...
export const isCiAlreadyAssigned = (ciUuid?: string, ciData?: ConfigurationItemUi[]) => {
    return ciData?.some((ci) => ci?.uuid === ciUuid)
}

export const calcBlockedPublicAuthorities = (
        ...
        configurationItemSet?.configurationItemSet?.map((confItem) => {
            ...
            if (confItem?.uuid === entityId)
                return {
                    ...confItem,
                    blocked: true,
                    blockMessage: t?.('publicAuthorities.assigned.warningInfo'),
                }
            else if (icoOfDetailOrg === TOP_LEVEL_PO_ICO) {
                return {
                    ...confItem,
                    blocked: true,
                    blockMessage: t?.('publicAuthorities.assigned.topLevelOrg'),
                }
            }
            ...
            if (foundInIntegrityData?.cycleInHierarchy) {
                return {
                    ...confItem,
                    blocked: true,
                    blockMessage: t?.('publicAuthorities.assigned.isCycleInHierarchy'),
                }
            } else if (foundInIntegrityData?.assignedInHierarchy) {
                return {
                    ...confItem,
                    blocked: true,
                    blockMessage: t?.('publicAuthorities.assigned.isAssigned'),
                }
            }
