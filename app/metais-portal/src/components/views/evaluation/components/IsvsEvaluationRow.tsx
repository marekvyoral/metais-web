...
export const IsvsEvaluationRow: React.FC<IIsvsEvaluationRowProps> = ({ uuid, entityId, isvsAttributes, dataRights, isGlobalAllowed }) => {
    ...
    const onSubmit = (formData: IDetailISVSColumn[]) => {
        ...
        if (dataRights?.inEvaluation) {
            ...
            saveResponseIsvsHook(entityId, uuid ?? '', 'ISVS', fetchData)
                .then(() => {
                    refetch()
                    setResultSuccessApiCall({ isSuccess: true, message: t('evaluation.saveSuccess') })
                })
