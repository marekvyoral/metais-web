...
export const RefDatasetInfoView: React.FC<RefIdentifierDetailInfoViewProps> = ({ ciItemData, attributes, registrationState, gestorName }) => {
    const {
        t,
        i18n: { language },
    } = useTranslation()
    ...
    return (
        <>
            ...
            <InformationGridRow
                ...
                value={ciItemData?.attributes?.[ATTRIBUTE_NAME.Profil_URIDataset_uri_datasetu]}
            />
            ...
            <InformationGridRow
                ...
                value={ciItemData?.attributes?.[ATTRIBUTE_NAME.Profil_URIDataset_historicky_kod]}
            />

            {ciItemData?.attributes?.[ATTRIBUTE_NAME.Profil_URIDataset_platne_od] && (
                <InformationGridRow
                    hideIcon
                    label={getNameByAttribute(
                        language,
                        attributes?.find((item) => item.technicalName === ATTRIBUTE_NAME.Profil_URIDataset_platne_od),
                    )}
                    value={t('date', { date: ciItemData?.attributes?.[ATTRIBUTE_NAME.Profil_URIDataset_platne_od] })}
                />
            )}

            {ciItemData?.attributes?.[ATTRIBUTE_NAME.Profil_URIDataset_platne_do] && (
                <InformationGridRow
                    hideIcon
                    label={getNameByAttribute(
                        language,
                        attributes?.find((item) => item.technicalName === ATTRIBUTE_NAME.Profil_URIDataset_platne_do),
                    )}
                    value={t('date', { date: ciItemData?.attributes?.[ATTRIBUTE_NAME.Profil_URIDataset_platne_do] })}
                />
            )}
