...
import { Button, ButtonLink, Filter, TextHeading } from '@isdd/idsk-ui-kit/index'
...
export const KRISDocumentsTable: React.FC<KRISDocumentsTable> = ({
    data,
    additionalColumns,
    isLoading,
    isError,
    ...
    const downloadProtocolRequest = useGetProtokolPdfHook()
    const [successfullyAdded, setSuccessfullyAdded] = useState<string[]>([])

    const downloadProtocol = async (ci: ConfigurationItemUi) => {
        const pdf = (await downloadProtocolRequest({ krisUuid: ciData?.uuid, protokolUuid: ci.uuid })) as unknown as Blob
        downloadBlobAsFile(new Blob([pdf]), ci?.attributes?.Gen_Profil_nazov + '.pdf', false)
    }
            ...
            : []),
        {
            ...
            meta: {
                getCellContext: (ctx: CellContext<ConfigurationItemUi, unknown>) =>
                    (ctx?.getValue?.() as ConfigurationItemUi).attributes?.Gen_Profil_nazov,
            },
            cell: (row) => {
    ...
    const protocolsColumns: Array<ColumnDef<ConfigurationItemUi>> = [
        {
            accessorFn: (row) => row,
            header: t('documentsTab.protocols.protocol'),
            id: 'documentsTab.protocols.protocol',
            size: 300,
            cell: (row) => {
                const ci = row.getValue() as ConfigurationItemUi
                return (
                    <Link to={''} onClick={async () => downloadProtocol(ci)}>
                        {ci?.attributes?.Gen_Profil_nazov}
                    </Link>
                )
            },
            meta: {
        ...
        },
        {
            ...
            header: t('documentsTab.protocols.description'),
            id: 'documentsTab.protocols.description',
            size: 200,
            meta: {
                getCellContext: (ctx: CellContext<ConfigurationItemUi, unknown>) => ctx.getValue() as string,
            },
        },
        {
            accessorKey: 'bulkActions',
            ...
            size: 130,
            cell: (row) => {
                const ci = row.cell.row.original as ConfigurationItemUi

                return <Button label={t('download')} onClick={() => downloadProtocol(ci)} />
            },
        },
    ]
    useEffect(() => {
    ...
    return (
        <QueryFeedback loading={isLoading || isBulkLoading} error={isError} indicatorProps={{ layer: 'parent' }} withChildren>
            ...
            <TextHeading size="L">{t('ciType.documents')}</TextHeading>
            <Table<TableCols> rowSelection={rowSelection} onRowSelectionChange={setRowSelection} columns={columns} data={data} />
            <PaginatorWrapper {...pagination} handlePageChange={handleFilterChange} />
            <Spacer vertical />
            <TextHeading size="L">{t('ciType.protocols')}</TextHeading>
            <Table<ConfigurationItemUi> columns={protocolsColumns} data={protocols} />
        </QueryFeedback>
    )
}
