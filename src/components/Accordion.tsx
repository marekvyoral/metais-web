...
const createAssociativeArray: (length: number, defaultValue: boolean) => { [key: string]: boolean } = (length, defaultValue) => {
    return Array.apply(false, Array(length)).reduce<{ [key: string]: boolean }>((prev, _curr, index) => {
...
const AccordionContainer: React.FC<AccordionContainerProps> = ({ children, id }) => {
    ...
    const toggleAllExpanded = () => {
        if (isAllExpanded) {
            setExpandedChildIndexes(createAssociativeArray(childCount, false));
        } else {
            setExpandedChildIndexes(createAssociativeArray(childCount, true));
        }
    };
                        ...
                        <div className={'govuk-accordion__section' + (expanded ? ' govuk-accordion__section--expanded' : '')}>
                            <div className="govuk-accordion__section-header">
                                <h2 className="govuk-accordion__section-heading">
                                    ...
                                    <span className="govuk-accordion__icon" onClick={onToggle}></span>
                                </h2>
